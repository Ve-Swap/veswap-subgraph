...
function updateGaugeVotes(
  ...
  const gaugeCtr = GaugeAbi.bind(Address.fromString(gaugeAdr));
  log.warning('total weight' + totalWeight.toString(), []);
  if (totalWeight == BigDecimal.zero()) {
    gauge.expectedAmount = BigDecimal.zero();
    gauge.expectAPR = BigDecimal.zero();
    gauge.expectAPRDerived = BigDecimal.zero();
  } else {
